...
def ussd_handler():
    ...
    cursor = db_connection.cursor()
    cursor.execute("SELECT language, progress FROM registrations WHERE session_id = %s", (session_id,))
    row = cursor.fetchone()
    if row:
    ...
    if text == "":
        ...
        if language_selection == "1":
            ...
            response = "CON kwiyandikisha byagenze neza.\nMurakoze!"
    ...
    if progress >= 0:
        cursor.execute("UPDATE registrations SET progress = %s WHERE session_id = %s", (progress, session_id))
        db_connection.commit()
